...
namespace LiveSplit.Amnesia
{
    class GameMemory
    {
        ...
        private const int UNUSED_BYTE_OFFSET = 0xC78B2;
        ...
        bool TryGetGameProcess()
        {
            ...
            if (!p.WriteBytes(p.MainModuleWow64Safe().BaseAddress + 0xC7554, payload1.ToArray()))
                return false;
            ...
            if (!p.WriteBytes(p.MainModuleWow64Safe().BaseAddress + 0xC773E, payload2.ToArray()))
                return false;
